...
export const METADATA_FIELDS: ExportFieldDefinition[] = [
  ...
  },
  {
    key: 'success',
    label: 'Success',
    description: 'True when the verification run produced no failure verdict',
    path: 'metadata.failure',
    extractor: (result) => result.metadata.failure === null,
  },
  {
    key: 'failure_category',
    label: 'Failure Category',
    description: 'Leaf category identifying the specific failure mode (empty on success)',
    path: 'metadata.failure.category',
    defaultValue: '',
    extractor: (result) => result.metadata.failure?.category ?? '',
  },
  {
    key: 'failure_group',
    label: 'Failure Group',
    description: 'Top-level grouping of the failure category (empty on success)',
    path: 'metadata.failure.group',
    defaultValue: '',
    extractor: (result) => result.metadata.failure?.group ?? '',
  },
  {
    key: 'failure_stage',
    label: 'Failure Stage',
    description: 'Pipeline stage that produced the failure verdict (empty on success)',
    path: 'metadata.failure.stage',
    defaultValue: '',
    extractor: (result) => result.metadata.failure?.stage ?? '',
  },
  {
    key: 'failure_reason',
    label: 'Failure Reason',
    description: 'Human-readable reason attached to the failure verdict (empty on success)',
    path: 'metadata.failure.reason',
    defaultValue: '',
    extractor: (result) => result.metadata.failure?.reason ?? '',
  },
  {
    key: 'caveats',
    label: 'Caveats',
    description: 'Comma-separated list of informational flags attached to the run',
    path: 'metadata.caveats',
    defaultValue: '',
    extractor: (result) => (result.metadata.caveats ?? []).join(','),
  },
  {
    key: 'execution_time',
