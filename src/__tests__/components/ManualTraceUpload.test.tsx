...
describe('ManualTraceUpload', () => {
  ...
  it('renders upload component correctly', () => {
    ...
    expect(
      screen.getByText('Upload a JSON file with precomputed answer traces keyed by question hash.')
    ).toBeInTheDocument();
  ...
  it('shows CSV mapper description when templates are available', () => {
    const finishedTemplates: Array<[string, unknown]> = [['hash1', { question: 'Test question' }]];
