...
function App() {
  ...
  const handleExtractedQuestions = (questions: QuestionData) => {
    ...
    console.log(`✅ Extracted ${Object.keys(questions).length} questions. Use Template Generator to create answer templates.`);
  };
  ...
  const handleTemplatesGenerated = (combinedData: QuestionData) => {
    ...
    if (Object.keys(combinedData).length > 0) {
      ...
      console.log(`✅ Loaded ${Object.keys(combinedData).length} questions with successfully generated templates into curator`);
    } else {
  ...
  const handleResetAllData = () => {
    if (window.confirm('This will clear all data including extracted questions, templates, and progress. Are you sure?')) {
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-4xl font-bold bg-gradient-to-r from-slate-800 via-blue-900 to-indigo-900 
          ...
          {import.meta.env.DEV && (
            <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-700 rounded-lg">
              <div className="text-xs text-yellow-800 dark:text-yellow-300">
                <strong>Dev Mode:</strong> Session ID: {sessionId.slice(-8)} | 
                Data: {Object.keys(questionData).length} questions, {Object.keys(checkpoint).length} checkpoint items, {Object.keys(extractedQuestions).length} extracted
                <button
                  onClick={() => {
                    forceResetAllData();
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  <option value="">
                    {questionIds.length === 0 ? 'No questions available - upload data first' : 'Choose a question...'}
                  </option>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 dark:border-slate-700/30 p-6">
                ...
                <div className="bg-slate-50/80 dark:bg-slate-700/80 backdrop-blur-sm rounded-xl p-4 border border-slate-100 dark:border-slate-600 shadow-inner">
                  <p className="text-slate-800 dark:text-slate-200 leading-relaxed font-medium">{selectedQuestion.question}</p>
                </div>
              </div>
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 dark:border-slate-700/30 p-6">
                ...
                <div className="bg-slate-50/80 dark:bg-slate-700/80 backdrop-blur-sm rounded-xl p-4 border border-slate-100 dark:border-slate-600 shadow-inner">
                  <p className="text-slate-800 dark:text-slate-200 leading-relaxed font-medium">{selectedQuestion.raw_answer}</p>
                </div>
              </div>
            ...
            {!selectedQuestion && !isLoading && (
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 dark:border-slate-700/30 p-12 text-center">
                ...
                {questionIds.length === 0 && Object.keys(checkpoint).length === 0 && Object.keys(extractedQuestions).length === 0 && (
                  <button
                    onClick={() => setActiveTab('extractor')}
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-xl">
                      <p className="text-blue-800 dark:text-blue-300 text-sm font-medium">
                        ✅ You have {Object.keys(extractedQuestions).length} extracted questions. Now generate answer templates for them.
                      </p>
                    </div>
                    <button
                      onClick={() => setActiveTab('generator')}
                ...
                {questionIds.length === 0 && Object.keys(checkpoint).length > 0 && (
                  <div className="mt-4 p-4 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-xl">
                    <p className="text-amber-800 dark:text-amber-300 text-sm font-medium">
                      💡 <strong>Checkpoint Loaded:</strong> You have {Object.keys(checkpoint).length} items in your checkpoint. 
                      Upload the Question Data JSON file to continue working with your saved progress.
                    </p>
                  </div>
                )}
              </div>
