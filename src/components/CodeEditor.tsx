...
export const CodeEditor = forwardRef<CodeEditorRef, CodeEditorProps>(
  ({ value, onChange, onSave, readOnly = false, originalCode = '', savedCode = '', enableFormEditor = false }, ref) => {
    const textareaRef = useRef<HTMLTextAreaElement>(null);
    const preRef = useRef<HTMLPreElement>(null);
    const containerRef = useRef<HTMLDivElement>(null);
    const formEditorRef = useRef<PydanticFormEditorRef>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        saveAllUnsavedFields: () => {
          if (formEditorRef.current && editorMode === 'form') {
            formEditorRef.current.saveAllUnsavedFields();
          }
        },
        hasUnsavedChanges: () => {
          if (formEditorRef.current && editorMode === 'form') {
            return formEditorRef.current.hasUnsavedChanges();
          }
          return false;
        },
      }),
