    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded shadow-lg max-h-32 overflow-y-auto">
          {options.map(option => (
            <label key={option} className="flex items-center px-2 py-1 hover:bg-slate-100 dark:hover:bg-slate-600 cursor-pointer">
              <input
                type="checkbox"
...
export const BenchmarkTable: React.FC<BenchmarkTableProps> = ({ 
  ...
  const getUniqueModels = () => {
    try {
      ...
      const answeringModels = new Set(allResults.map(r => r.answering_model).filter((model): model is string => Boolean(model)));
      const parsingModels = new Set(allResults.map(r => r.parsing_model).filter((model): model is string => Boolean(model)));
      const runNames = new Set(allResults.map(r => r.run_name).filter((name): name is string => Boolean(name)));
      const replicates = new Set(allResults.map(r => {
              ...
              {table.getHeaderGroups().map(headerGroup => (
                <tr key={headerGroup.id} className="border-b border-slate-200 dark:border-slate-600">
                  {headerGroup.headers.map(header => (
                    <th key={header.id} colSpan={header.colSpan} className={`py-2 px-3 text-sm font-medium text-slate-700 dark:text-slate-300 ${header.id === 'index' ? 'w-16 text-center' : 'text-left'}`}>
                      {header.isPlaceholder ? null : (
                        <div className="space-y-2">
                          ...
                          {['answering_model', 'parsing_model', 'run_name', 'replicate'].includes(header.id) && (
                            <MultiSelectFilter
                              options={
                                header.id === 'answering_model' ? answeringModels :
                                header.id === 'parsing_model' ? parsingModels :
                                header.id === 'run_name' ? runNames : replicates
                              }
                          ...
                          {header.id === 'execution_time' && (
                            <div className="flex gap-1">
                              <input
                                ...
                                onChange={(e) => header.column.setFilterValue((old: [number, number]) => [e.target.value ? Number(e.target.value) : undefined, old?.[1]])}
                              ...
                              <input
                                ...
                                onChange={(e) => header.column.setFilterValue((old: [number, number]) => [old?.[0], e.target.value ? Number(e.target.value) : undefined])}
                                className="w-12 px-1 py-1 text-xs border border-slate-300 dark:border-slate-600 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100"
                                step="0.1"
                              />
                            </div>
                           ...
                           {header.id === 'timestamp' && (
                            <div className="space-y-1">
                              <input
                                ...
                                onChange={(e) => header.column.setFilterValue((old: [string, string]) => [e.target.value, old?.[1]])}
                              ...
                              <input
                                ...
                                onChange={(e) => header.column.setFilterValue((old: [string, string]) => [old?.[0], e.target.value])}
                                className="w-full px-1 py-1 text-xs border border-slate-300 dark:border-slate-600 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100"
                              />
                            </div>
                          )}
  ...
  // Export the reset function for parent components
